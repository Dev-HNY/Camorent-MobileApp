...
import { Pressable, Dimensions, ScrollView, View } from "react-native";
...
export function HeroSection({ onSearchPress }: HeroSectionProps) {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      const next = (activeSlideRef.current + 1) % SLIDE_COUNT;
    ...
    <YStack>
      ...
      >
        ...
        <XStack alignItems="center" gap={8} flex={1}>
          ...
          <Pressable onPress={() => setShowCityModal(true)} style={{ flex: 1 }}>
            <YStack>
        ...
        <XStack alignItems="center" gap={12}>
          <Pressable onPress={onSearchPress || (() => router.push("/search"))}>
            <SearchIconCircle />
          </Pressable>
          <Pressable
            onPress={() =>
      ...
      {/* Hero Image Carousel */}
      <View style={{ marginTop: 120 + insets.top }}>
        <ScrollView
          ref={scrollRef}
          horizontal
        ...
        >
          {HERO_IMAGES.map((src, idx) => (
            <View key={idx} style={{ width: SCREEN_WIDTH, paddingHorizontal: 16 }}>
              <Image
                source={src}
                contentFit="cover"
                cachePolicy="memory-disk"
                priority={idx === 0 ? "high" : "normal"}
                style={{ height: 196, width: SCREEN_WIDTH - 32, borderRadius: 10 }}
              />
            </View>
          ))}
        </ScrollView>
      ...
      {/* Carousel Indicators */}
      <View style={{ backgroundColor: "white", paddingVertical: 12, flexDirection: "row", justifyContent: "center", gap: 8 }}>
        {HERO_IMAGES.map((_, idx) => (
          <View
            key={idx}
            style={{
              width: activeSlide === idx ? 32 : 11.27,
              height: 7.51,
              backgroundColor: activeSlide === idx ? "#8E0FFF" : "#EBEBEF",
              borderRadius: 3.76,
            }}
          />
        ))}
